#!/usr/bin/env python3
"""
EMERGENCY FIXED: Process 200 proteins quickly
"""
import os
import torch
...
class EmergencyBuilderFixed:
    def __init__(self, cutoff=8.0):  # FIXED: parameter name
        self.cutoff = cutoff
        self.residue_types = {
    ...
    def create_labels(self, residues, ligand_file):
        ...
        try:
            with open(ligand_file, 'r') as f:
                for line in f:
                    if line.startswith('HETATM') or (len(line.split()) >= 4 and line[0].isdigit()):
                        parts = line.split()
                        try:
                            if len(parts) >= 3:
        ...
        for res in residues:
            ...
            try:
                distances = np.sqrt(((res_coords[:, np.newaxis, :] - lig_coords) ** 2).sum(axis=2))
...
def process_200_proteins():
    ...
    builder = EmergencyBuilderFixed(cutoff=8.0)  # FIXED: use correct parameter
    ...
    print(f"\nSuccessfully processed {success_count}/{len(target_ids)} proteins")
    ...
    stats = {
        ...
        'success_rate': success_count/len(target_ids),
