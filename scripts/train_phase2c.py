"""
Train Hamiltonian GNN with CORRECT physics weight
"""
import os
import torch
...
def train_hamiltonian_fixed():
    """Train with proper physics weight scaling"""
    print("=" * 60)
    print("FIXED HAMILTONIAN GNN TRAINING")
    print("=" * 60)
    ...
    # FIXED HYPERPARAMETERS - SMALL physics weight!
    hyperparams = {
        'input_dim': input_dim,
        'hidden_dim': 128,
        'num_layers': 3,
        ...
        'learning_rate': 0.001,
        'weight_decay': 1e-5,
        'physics_weight': 0.0001,  # FIXED: 1000× smaller than before!
        'pos_weight': pos_weight.item() if torch.is_tensor(pos_weight) else pos_weight
    }
    
    print("\nFIXED Hyperparameters (physics_weight 1000× smaller):")
    for k, v in hyperparams.items():
        print(f"  {k}: {v}")
    
    # Create model with FIXED physics weight
    model = HamiltonianGNN(
        input_dim=hyperparams['input_dim'],
        hidden_dim=hyperparams['hidden_dim'],
        num_layers=hyperparams['num_layers'],
        dropout=hyperparams['dropout'],
        physics_weight=hyperparams['physics_weight']  # Now 0.0001
    )
    
    # Create trainer with FIXED physics weight
    ...
    save_dir = "./experiments/results/phase2b/week2/training_fixed"
    ...
    # Train with FIXED physics weight
    ...
    # Calculate physics contribution percentage
    if val_total > 0:
        phys_contribution = (hyperparams['physics_weight'] * val_phys) / val_total * 100
    else:
        phys_contribution = 0
    
    print(f"  Physics Contribution: {phys_contribution:.1f}%")
    ...
    performance = {
        ...
        'physics_contribution_percent': float(phys_contribution),
        'num_train_graphs': len(train_graphs),
        'num_val_graphs': len(val_graphs)
    }
    
    with open(os.path.join(save_dir, 'hamgnn_performance_fixed.json'), 'w') as f:
        json.dump(performance, f, indent=2)
    
    print(f"\n✓ Performance saved to: {os.path.join(save_dir, 'hamgnn_performance_fixed.json')}")
    
    # Check if target achieved
    target_f1 = 0.35
    if val_f1 >= target_f1:
        print(f"\n🎉 TARGET ACHIEVED! F1 = {val_f1:.4f} >= {target_f1:.4f}")
        print(f"   Physics contribution: {phys_contribution:.1f}% (good: 5-20%)")
        return True, performance
    elif val_f1 >= base_f1:
        print(f"\n⚠️  IMPROVED but target not met: F1 = {val_f1:.4f} < {target_f1:.4f}")
        print(f"   Improvement: {val_f1 - base_f1:+.4f} over baseline")
        print(f"   Physics contribution: {phys_contribution:.1f}%")
        return False, performance
    else:
        print(f"\n❌ WORSE than baseline: F1 = {val_f1:.4f} < {base_f1:.4f}")
        print("   Physics is hurting performance!")
        print("   Try even smaller physics_weight or fix physics loss calculation")
        return False, performance

if __name__ == "__main__":
    success, performance = train_hamiltonian_fixed()
