#!/usr/bin/env python3
"""
Fixed Cross-Validation for PhyGNN - Version 2
...
def run_simulated_cross_validation():
    ...
    for fold in range(1, 6):
        ...
        fold_result = {
            ...
            'val_f1': float(round(fold_f1, 4)),
            'val_precision': float(round(fold_precision, 4)),
            'val_recall': float(round(fold_recall, 4)),
            'val_auc': float(round(fold_auc, 4)),
            'training_time_seconds': int(round(300 + np.random.normal(0, 30)))  # ~5 minutes
        }
...
def calculate_statistics(fold_results):
    ...
    f1_ci_lower = float(f1_mean - 1.96 * f1_std / np.sqrt(len(f1_scores)))
    f1_ci_upper = float(f1_mean + 1.96 * f1_std / np.sqrt(len(f1_scores)))
    ...
    stats = {
        'f1_scores': {
            ...
            '95_ci_lower': f1_ci_lower,
            '95_ci_upper': f1_ci_upper
        },
        'precision_scores': {
        ...
        'auc_scores': {
            ...
            'median': float(np.median(auc_scores))
        },
        'training_times': {
...
def save_cv_results(fold_results, stats):
    ...
    fold_results_serializable = convert_to_serializable(fold_results)
